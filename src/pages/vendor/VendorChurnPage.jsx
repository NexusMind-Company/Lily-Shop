...
const DiscountModal = ({ customer, onSend, onClose, isSending }) => {
  ...
  const handleSend = () => {
    if (!msg.trim()) { toast.error("Please write a message."); return; }
    onSend({ discount_percentage: discount, message: msg.trim(), expiry_days: 7 });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-surface-dark rounded-t-3xl p-5 pb-8 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-bold text-[#111813] dark:text-white">Win-back Offer</h3>
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3 mb-4 flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-[#4eb75e]/10 flex items-center justify-center text-sm font-bold text-[#4eb75e]">
            {customer.customer_name?.charAt(0) ?? "?"}
          </div>
          <div>
            <p className="text-sm font-bold text-[#111813] dark:text-white">{customer.customer_name}</p>
            <p className="text-xs text-gray-400">Left {customer.days_since_cancel} days ago</p>
          </div>
        </div>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-semibold text-gray-500 mb-2 block">Discount Amount</label>
            <div className="flex gap-2">
              {[5, 10, 15, 20, 25].map((d) => (
                <button key={d} onClick={() => setDiscount(d)}
                  className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${discount === d ? "bg-[#4eb75e] text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-500"}`}>
                  {d}%
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs font-semibold text-gray-500 mb-1.5 block">Personal Message</label>
...
const VendorChurnPage = () => {
  ...
  const { mutate: sendOffer, isPending: sendingOffer } = useMutation({
    mutationFn: ({ customerId, offerData }) => sendWinbackOffer(customerId, offerData),
    onSuccess: () => { setOfferTarget(null); toast.success("Win-back offer sent! 🎉"); },
    onError: (err) => toast.error(getErrorMessage(err)),
  });

  if (isLoading && !churnData) return <VendorLayout title="Churn Tracking"><VendorPageLoader /></VendorLayout>;
  if (isError && !churnData) return <VendorLayout title="Churn Tracking"><VendorPageError message={getErrorMessage(error)} onRetry={refetch} /></VendorLayout>;
    ...
    <VendorLayout title="Churn Tracking">
      <div className="space-y-4">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 rounded-2xl p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-2xl font-bold text-red-600 dark:text-red-400">{churnData?.count ?? 0}</p>
            <p className="text-xs text-red-500">Customers didn't renew</p>
          </div>
        </div>

        <div className="flex gap-2">
          {[{ key: "this_week", label: "This Week" }, { key: "this_month", label: "This Month" }, { key: "all_time", label: "All Time" }].map(({ key, label }) => (
            <button key={key} onClick={() => setPeriod(key)}
              className={`flex-1 py-2 rounded-xl text-xs font-semibold transition-all ${period === key ? "bg-[#111813] dark:bg-white text-white dark:text-[#111813]" : "bg-white dark:bg-surface-dark border border-gray-100 dark:border-gray-800 text-gray-500"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {churned.length === 0 ? (
            ...
            churned.map((c) => (
              <div key={c.id} className="bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-sm font-bold text-gray-400 flex-shrink-0">
                  {c.customer_name?.charAt(0) ?? "?"}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-[#111813] dark:text-white truncate">{c.customer_name}</p>
                  <p className="text-xs text-gray-400 truncate">{c.last_plan}</p>
                  <div className="flex items-center gap-1 mt-0.5 text-[10px] text-gray-400">
                    <Clock size={10} />{c.days_since_cancel} days ago
                  </div>
                </div>
                <button onClick={() => setOfferTarget(c)}
      ...
      {offerTarget && (
        <DiscountModal
          ...
          onSend={(data) => sendOffer({ customerId: offerTarget.id, offerData: data })}
