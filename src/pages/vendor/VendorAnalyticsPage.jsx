    ...
    <VendorLayout title="Analytics">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={label} className="bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col gap-2">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${color}`}><Icon size={18} className="text-white" /></div>
      ...
      <div className="bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800">
        ...
        <div className="flex items-center gap-4">
          <PieChart width={120} height={120}>
            <Pie data={[{ value: a.retention_rate ?? 0 }, { value: a.churn_rate ?? 0 }]}
              cx="50%" cy="50%" innerRadius={35} outerRadius={55} dataKey="value" startAngle={90} endAngle={-270}>
              <Cell fill="#4eb75e" />
              <Cell fill="#fee2e2" />
            </Pie>
          </PieChart>
          <div className="flex-1 space-y-3">
            {[
              { label: "Retention", value: `${a.retention_rate ?? 0}%`, color: "#4eb75e", bg: "bg-[#4eb75e]" },
              { label: "Churn", value: `${a.churn_rate ?? 0}%`, color: "#f87171", bg: "bg-red-200" },
              ...
              <div key={label}>
                ...
                <p className="text-xl font-bold ml-4" style={{ color }}>{value}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          (a.meal_popularity ?? []).map((meal, i) => (
            <div key={meal.meal_name} className="mb-3">
              <div className="flex justify-between items-center mb-1">
                <p className="text-xs font-semibold text-[#111813] dark:text-white">{meal.meal_name}</p>
                <p className="text-xs font-bold text-[#4eb75e]">{meal.percentage}%</p>
              </div>
              <div className="w-full h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${meal.percentage}%`, backgroundColor: COLORS[i % COLORS.length] }} />
              </div>
              <p className="text-[10px] text-gray-400 mt-0.5">{meal.order_count} orders</p>
            </div>
          ))
        )}
      </div>
      ...
      <div className="bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800">
        ...
        <p className="text-xs text-gray-500 mb-3">What customers frequently ask to remove</p>
        {(a.ingredient_insights ?? []).length === 0 ? (
          ...
          <>
            {(a.ingredient_insights ?? []).map((item) => (
              <div key={item.ingredient} className="flex items-center gap-3 mb-3">
                <p className="text-sm font-semibold text-[#111813] dark:text-white w-20 flex-shrink-0">{item.ingredient}</p>
                <div className="flex-1 h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                  <div className="h-full bg-orange-400 rounded-full" style={{ width: `${item.removal_rate ?? 0}%` }} />
                </div>
                <p className="text-xs font-bold text-orange-500 w-10 text-right">{item.removal_rate}%</p>
              </div>
            ))}
            {(a.ingredient_insights ?? [])[0] && (
              <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl px-3 py-2 mt-2">
                <p className="text-xs text-yellow-700 dark:text-yellow-400">
                  💡 {a.ingredient_insights[0].removal_rate}% of customers remove {a.ingredient_insights[0].ingredient?.toLowerCase()}.
      ...
      <div className="bg-white dark:bg-surface-dark rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
        ...
        {(a.top_performing_plans ?? []).length === 0 ? (
          <p className="text-xs text-gray-400 text-center py-6">No plan data yet</p>
        ) : (
          (a.top_performing_plans ?? []).map((plan, i) => (
            <div key={plan.plan_name} className="flex items-center gap-3 px-4 py-3 border-b border-gray-50 dark:border-gray-800 last:border-0">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 ${i === 0 ? "bg-yellow-400" : i === 1 ? "bg-gray-400" : "bg-orange-400"}`}>{i + 1}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#111813] dark:text-white truncate">{plan.plan_name}</p>
                <p className="text-xs text-gray-400">{plan.subscribers} subscribers</p>
              </div>
              <p className="text-sm font-bold text-[#4eb75e] flex-shrink-0">₦{(plan.revenue ?? 0).toLocaleString()}</p>
            </div>
          ))
        )}
      </div>
