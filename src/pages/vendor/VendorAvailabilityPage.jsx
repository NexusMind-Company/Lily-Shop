...
const VendorAvailabilityPage = () => {
  ...
  const { mutate: save, isPending } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vendorAvailability"] });
  ...
  return (
    <VendorLayout title="Availability">
      <div className="space-y-5">
        ...
        <div className="bg-white  rounded-2xl p-4 shadow-sm border border-gray-100 ">
          <h3 className="text-sm font-bold text-[#111813]  mb-1">
            Max Meals Per Day
          </h3>
          <p className="text-xs text-gray-400 mb-3">
            Once this limit is reached, your shop shows "SOLD OUT"
          </p>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setMaxMeals((n) => Math.max(1, n - 5))}
              className="w-10 h-10 rounded-xl bg-gray-100  text-lg font-bold text-[#111813]  flex items-center justify-center"
            >
              −
            </button>
            <div className="flex-1 text-center">
              <p className="text-3xl font-bold text-[#111813] ">{maxMeals}</p>
              <p className="text-xs text-gray-400">meals/day</p>
            </div>
            <button
              onClick={() => setMaxMeals((n) => n + 5)}
              className="w-10 h-10 rounded-xl bg-gray-100  text-lg font-bold text-[#111813]  flex items-center justify-center"
            >
              +
            </button>
