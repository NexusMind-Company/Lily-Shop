...
const PRICE_RANGES = {
  small: {
    ...
    max: 2000,
    label: "Small Meal",
    example: "Snacks, light bites",
  },
  medium: {
    min: 2000,
    max: 3500,
    label: "Medium Meal",
    example: "Jollof Rice, Fried Rice",
  },
  large: {
    min: 3500,
    max: 5000,
    label: "Large Meal",
    example: "Full combos, special packs",
  },
};
const SIZE_COLORS = {
  ...
  <div className="fixed inset-0 z-50 flex items-end justify-center">
    ...
    <div className="relative w-full max-w-md bg-white  rounded-t-3xl p-6 pb-8 shadow-2xl">
      <div className="flex items-center justify-between mb-5">
        <h3 className="font-bold text-[#111813] ">Pricing Guidelines</h3>
        <button
          onClick={onClose}
      ...
      <p className="text-xs text-gray-500 mb-4">
        Prices must fall within these ranges to keep meals affordable for
        customers.
      </p>
      {Object.entries(PRICE_RANGES).map(
        ([size, { label, min, max, example }]) => (
          <div
            key={size}
            className={`rounded-xl border p-3 mb-2 ${SIZE_COLORS[size]}`}
          >
...
const FoodGuidelines = ({ onClose }) => (
  <div className="fixed inset-0 z-50 flex items-end justify-center">
    ...
    <div className="relative w-full max-w-md bg-white  rounded-t-3xl p-6 pb-8 shadow-2xl">
      <div className="flex items-center justify-between mb-5">
        <h3 className="font-bold text-[#111813] ">Food Quality Standards</h3>
        <button
          onClick={onClose}
          className="p-1.5 rounded-full hover:bg-gray-100 "
        >
        ...
        {[
          {
            emoji: "💰",
            title: "Affordable",
            desc: "Price must fall within the approved range for your meal size.",
          },
          {
            emoji: "😋",
            title: "Delicious",
            desc: "Well-seasoned, properly cooked Nigerian dishes.",
          },
          {
            emoji: "🧼",
            title: "Hygienically Prepared",
            desc: "Clean environment, fresh ingredients daily.",
          },
          {
            emoji: "🍛",
            title: "Popular Nigerian Dishes",
            desc: "Jollof Rice, Egusi, Fried Rice, Pepper Soup, etc.",
          },
          {
            emoji: "📏",
            title: "Correct Portion Sizing",
            desc: "Portions must match the selected size category.",
          },
        ].map(({ emoji, title, desc }) => (
          <div key={title} className="flex gap-3 p-3 bg-gray-50  rounded-xl">
            <span className="text-xl">{emoji}</span>
            <div>
              <p className="text-sm font-bold text-[#111813] ">{title}</p>
...
const MealForm = ({ meal, onSave, onCancel, isSaving }) => {
  ...
  const priceValid =
    !form.price || (priceNum >= range.min && priceNum <= range.max);
        ...
        <div className="relative w-full max-w-md bg-white  rounded-t-3xl p-6 pb-8 shadow-2xl">
          <div className="text-center mb-5">
            ...
            <h3 className="font-bold text-[#111813]  text-lg">
              Before You Add a Meal
            </h3>
            <p className="text-xs text-gray-500 mt-1">
              Please review our food standards
            </p>
          </div>
          <div className="space-y-2 mb-5">
            {[
              "Meals must be affordable & properly priced",
              "Nigerian dishes preferred (Jollof, Egusi, etc.)",
              "Hygienically prepared with fresh ingredients",
              "Portions must match the selected size category",
            ].map((point) => (
              <div
                key={point}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      <div
        ...
        onClick={onCancel}
      />
      <div className="relative w-full max-w-md bg-white  rounded-t-3xl p-5 pb-8 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <p className="text-xs text-gray-400">
              {form.image ? form.image.name : "Tap to upload meal image"}
            </p>
            <input
              ref={fileRef}
              type="file"
          ...
          <div>
            ...
            {priceWarning && (
              <p className="flex items-center gap-1 text-xs text-red-500 mt-1.5">
                ...
                Price must be ₦{range.min.toLocaleString()} – ₦
                {range.max.toLocaleString()} for {form.size_category} meals
              </p>
            )}
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="e.g. Rice, Chicken, Tomatoes, Onions"
    ...
    <VendorLayout title="Menu">
      <div className="flex gap-2">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl border border-gray-100  bg-white  text-xs font-semibold text-gray-500"
        >
      ...
      {editMeal && (
        <MealForm
          meal={editMeal}
          isSaving={updating}
          onSave={(d) => update({ id: editMeal.id, data: d })}
          onCancel={() => setEditMeal(null)}
        />
      )}
      {showGuidelines && (
        <FoodGuidelines onClose={() => setShowGuidelines(false)} />
      )}
    </VendorLayout>
  );
};
