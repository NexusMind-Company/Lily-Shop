...
const VendorAddonsPage = () => {
  ...
  const { mutate: add, isPending: adding } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vendorAddons"] });
      setShowForm(false);
  ...
  const toggleAvailability = (addon) =>
    update({ id: addon.id, data: { is_available: !addon.is_available } });
    ...
    <VendorLayout title="Add-ons">
      <div className="bg-lily/10 border border-lily/20 rounded-2xl px-4 py-3">
        ...
        <p className="text-xs text-gray-500">
          Optional extras customers can add with their meal subscriptions —
          drinks, desserts, weekend treats.
        </p>
      </div>
      ...
      {editAddon && (
        <AddonForm
          addon={editAddon}
          isSaving={updating}
          onSave={(d) => update({ id: editAddon.id, data: d })}
