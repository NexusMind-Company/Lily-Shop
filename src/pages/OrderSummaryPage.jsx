    ...
    <div className="flex flex-col min-h-screen max-w-xl mx-auto bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm">
          <h3 className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-3">Delivery To</h3>
          <div className="flex items-start space-x-3">
            <MapPin className="text-lily mt-0.5" size={20} />
            <div>
              <p className="font-bold text-gray-800">{selectedAddress?.name}</p>
              <p className="text-gray-600 text-sm leading-relaxed">{selectedAddress?.address}</p>
        ...
        {/* Payment Method Card */}
        <div className="bg-white p-4 rounded-xl shadow-sm">
          <h3 className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-3">Payment Method</h3>
        ...
        {/* Order Items Preview */}
        <div className="bg-white p-4 rounded-xl shadow-sm">
          <h3 className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-3">Items ({cartItems.length})</h3>
          <div className="space-y-4">
            {cartItems.map((item) => (
              <div key={item.id} className="flex justify-between items-center">
                <div className="flex items-center space-x-3">
                  <div className="w-12 h-12 bg-gray-100 rounded-md overflow-hidden">
                    <img src={item.mediaSrc} alt="" className="w-full h-full object-cover" />
                  </div>
                  <div>
                    <p className="font-medium text-gray-800 text-sm line-clamp-1 w-40">{item.productName}</p>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm space-y-2">
          ...
          <div className="border-t border-gray-100 pt-2 mt-2 flex justify-between items-center">
            ...
            <span className="font-bold text-lily text-xl">{formatMoney(TOTAL_PAYABLE)}</span>
          </div>
        </div>
      </div>
