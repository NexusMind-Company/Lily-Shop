    ...
    <div className="flex flex-col min-h-screen max-w-xl mx-auto bg-white">
      ...
      {data && (
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
           {data.length === 0 ? (
             <p className="text-center text-gray-500 mt-8">No saved cards found.</p>
           ) : (
             data.map((card) => (
                <div
                key={card.id}
                ...
                className={`flex items-center justify-between p-4 border rounded-lg cursor-pointer ${selectedCardId === card.id ? 'border-lily ring-1 ring-lily' : 'border-gray-200'}`}
                >
                <div className="flex items-center space-x-3">
                    {selectedCardId === card.id ? (
                    <CheckCircle2 size={24} className="text-lily flex-shrink-0" />
                    ) : (
