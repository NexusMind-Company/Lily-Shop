...
const BottomNav = ({ activePage }) => {
  return (
    <div className="flex md:hidden justify-around items-center bg-white h-15 pt-2 shadow-inner fixed bottom-0 left-0 w-full z-50">
      {/* Home  */}
      <Link to="/" className={`flex flex-col items-center relative ${activePage === "home" ? "text-lily" : "text-ash"}`}>
      ...
      {/* Create */}
      <Link to="/createContent" className={`flex flex-col items-center relative ${activePage === "create" ? "text-lily" : "text-ash"}`}>
        <button className="grid place-items-center size-10 absolute -top-3 transform -translate-x-1/2 left-1/2">
          {activePage === "create" ? (
      ...
      {/* Food */}
      <Link to="/food" className={`flex flex-col items-center relative ${activePage === "food" ? "text-lily" : "text-ash"}`}>
        <button className="grid place-items-center size-10 absolute -top-3 transform -translate-x-1/2 left-1/2">
          {activePage === "food" ? (
      ...
      {/* Chatroom */}
      <Link to="/inbox" className={`flex flex-col items-center relative ${activePage === "inbox" ? "text-lily" : "text-ash"}`}>
        <button className="grid place-items-center size-10 absolute -top-3 transform -translate-x-1/2 left-1/2">
          {activePage === "inbox" ? (
            <div className="relative">
              <img src="/icons/message-3-active.svg" className="text-lily h-7 w-7" />
      ...
      {/* Profile */}
      <Link to="/profile" className={`flex flex-col items-center relative ${activePage === "profile" ? "text-lily" : "text-ash"}`}>
        <button className="grid place-items-center size-10 absolute -top-3 transform -translate-x-1/2 left-1/2">
          {activePage === "profile" ? (
