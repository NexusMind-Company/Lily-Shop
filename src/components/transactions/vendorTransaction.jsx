...
export default function VendorTransaction() {
  // Mock recent vendor transactions (design-first; no backend wiring)
  const now = Date.now();
  const [transactions] = useState([
    {
      id: "v1",
      title: "Product sale",
      description: "Order ORD-2043 payout (gross ₦10,000 − fee ₦500)",
      direction: "in",
      amount: 9500,
    ...
    },
    {
      ...
      direction: "out",
      amount: 10000,
      createdAt: now - 10 * 60 * 60 * 1000, // 10h ago
    },
    {
      id: "v3",
      title: "Product sale",
      description: "Order ORD-2039 payout (gross ₦8,000 − fee ₦400)",
      direction: "in",
      amount: 7600,
    ...
    },
    {
      id: "v4",
      title: "Product sale",
      description: "Order ORD-2031 payout (gross ₦12,500 − fee ₦625)",
      direction: "in",
      amount: 11875,
      createdAt: now - 4 * 24 * 60 * 60 * 1000, // 4d ago
    },
    {
      id: "v5",
      title: "Withdrawal",
      ...
      direction: "out",
      amount: 5000,
      createdAt: now - 7 * 24 * 60 * 60 * 1000, // 7d ago
    },
  ]);
  ...
  // Filter and sort transactions (desc by date by default)
  const filtered = useMemo(() => {
    setError("");
    let items = [...transactions].sort((a, b) => b.createdAt - a.createdAt);

    if (!startDate && !endDate) return items; // initial load: show all recent
    ...
    <section className="max-w-4xl mx-auto px-3 sm:px-7 mt-6 sm:mt-10 mb-20 sm:mb-12">
      ...
      <div className="rounded-2xl border border-black h-12 sm:h-16 w-full flex items-center justify-center mb-4 sm:mb-6">
        <h1 className="text-base sm:text-lg md:text-xl font-normal font-poppins px-2 text-center">
          Vendor <span className="text-lily">Transactions</span>
        </h1>
      </div>
